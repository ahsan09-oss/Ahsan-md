...
${config.prefix}info 3660164118134`
      ...
      if (data.data.length > 10) {
        caption += `⚠️ Showing first 10 results. Use more specific query if needed.\n\n`;
      }

      caption += `🔎 _${query}_ Result From.\n`;
      caption += `🤖 *${botName}*`;

      // Prevent oversized captions
      if (caption.length > 3500) {
        caption = caption.slice(0, 3500);
        caption += '\n\n⚠️ Output trimmed due to size limit.';
      }
      ...
      if (fs.existsSync(imagePath)) {
        ...
        try {
          imageBuffer = {
            url: await sock.profilePictureUrl(
              sock.user.id,
              'image'
            )
          };
      ...
      await extra.reply(
        `❌ Failed to fetch information: ${error.message}`
      );
